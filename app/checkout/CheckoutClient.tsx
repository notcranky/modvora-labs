...
export default function CheckoutClient({ user }: CheckoutClientProps) {
  ...
  const isPaidPlan = selectedPlan.price > 0 && !isOwner
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0b] px-4 py-16">
        <div className="mx-auto max-w-md text-center">
          ...
          <p className="text-zinc-400 mb-6">Please sign in to continue with {selectedPlan.name}.</p>
          <Link
            href={`/signin?from=/checkout`}
            className="inline-block rounded-xl bg-purple-600 px-6 py-3 text-white font-medium hover:bg-purple-500 transition-colors"
          >
            Sign In to Continue
          </Link>
              ...
              <div className="space-y-5 mb-6">
                <div className="rounded-2xl border border-purple-500/20 bg-purple-600/5 p-4">
                  ...
                  <p className="text-zinc-400 text-sm leading-relaxed">
                    {isFreePlan
                      ? 'This path stays fast and useful so you can explore the platform before deciding on upgrades.'
                      : 'If this becomes a real purchase step later, the flow stays short: pick plan, confirm, and go to the dashboard.'}
                  </p>
                </div>

                {!isOwner && isFreePlan && (
                  <div className="rounded-2xl border border-[#2a2a30] bg-black/20 p-4">
                    <label className="block text-xs text-zinc-500 mb-2">Email for your build updates</label>
                    <input
                      type="email"
                      value={contactEmail}
                      onChange={(e) => setContactEmail(e.target.value)}
                      placeholder="you@email.com"
                      className="w-full rounded-xl border border-[#2a2a30] bg-[#18181f] px-3 py-2 text-sm text-white placeholder-zinc-600 outline-none focus:border-purple-500/50 transition-colors"
                    />
